...
- ISBN：97からはじまる13桁のISBN情報。共通本のID。
...
2) 本棚  
登録：本のISBNを入力し、本棚にISBNを登録する。同じ本棚に同じISBNの本は2つ以上存在しない。  
確認：本棚の一覧を表示する。  
...
【操作概要】  
1. 初期     
　ユーザ名、パスワードの各欄が空白になっている。    
2. 「ログインボタン」の押下    
　ユーザが確認ボタンを押下すると、必要な箇所(①および②)が入力されたことをチェックし、ユーザ名とパスワードに問題がなければ、メイン画面に遷移する。  
...
2. 「本の情報を取得」の押下  
　ユーザがボタンを押下すると、必要な箇所(①)が入力されたことをチェックし、問題がなければ、ISBNから書籍情報(③～⑦)を下の領域に出力する。  
※チェック内容：  
　ISBNコードが数字で構成されているか？  
　ISBN問い合わせして登録があるか？  
...
　①②がデフォルト値になっている。  
2. 「読書プランに登録する」の押下  
　ユーザがボタンを押下すると、必要な箇所(①②)が入力されたことをチェックし、問題がなければ、メイン画面(top.htm)に遷移する。  
　※チェック内容
